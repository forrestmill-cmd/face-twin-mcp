...
type Framing = keyof typeof BASE_URLS;
...
type StatusResponse = {
  status: string;
  revealed?: boolean;
  images?: Array<{ url: string; label: string; prompt_id: string }> | null;
  error?: string;
};

async function postJson<T>(url: string, origin: string, body?: unknown): Promise<T> {
...
async function getJson<T>(url: string, origin: string): Promise<T> {
  const response = await fetch(url, {
    headers: {
      Origin: origin,
      Referer: `${origin}/`,
...
async function createPrank(imageUrl: string, framing: Framing, waitForReady: boolean) {
  const baseUrl = BASE_URLS[framing];
  const upload = await postJson<UploadResponse>(`${baseUrl}/api/upload`, baseUrl, {
    url: imageUrl,
  });

  let status: StatusResponse = { status: upload.status };
  if (waitForReady) {
    await postJson(`${baseUrl}/api/prank/${upload.slug}/generate`, baseUrl);
    const deadline = Date.now() + 120_000;
    while (Date.now() < deadline) {
      status = await getJson<StatusResponse>(
        `${baseUrl}/api/prank/${upload.slug}/status`,
        baseUrl
      );
...
server.tool(
  ...
  "Create a FaceTwin / Prank My Face AI doppelganger prank link from a public image URL.",
  {
